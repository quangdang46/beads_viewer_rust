...
// Note: AGENT_BLURB, BLURB_START_MARKER, BLURB_END_MARKER are in the parent
// module but used only in blurb functions there (not in file operations).
...
pub fn append_blurb_to_file(path: &std::path::Path) -> Result<(), String> {
    ...
    atomic_write(path, new_content.as_bytes()).map_err(|e| format!("write file: {e}"))
}
...
pub fn create_agent_file(path: &std::path::Path) -> Result<(), String> {
    ...
    atomic_write(path, content.as_bytes()).map_err(|e| format!("create file: {e}"))
}
...
mod tests {
    ...
    fn create_and_verify() {
        let tmp = std::env::temp_dir().join("bvr_test_create_agent.md");
    ...
    fn append_and_remove_roundtrip() {
        let tmp = std::env::temp_dir().join("bvr_test_roundtrip.md");
        let _ = std::fs::remove_file(&tmp);
